...
for i = 1:3
    ...
    %     gridPos = amplitude * sin(angFreq * time + startPhase);
        ...
        if (direction == 1) && (xPosVector >= maxLimit) && (yPosVector == (maxLimit*-1))
            direction = 2;
            yPosVector = 0;
            pointReached = true;
            pause(0.5);
        ...
        if (direction == 2) && (xPosVector <= (maxLimit*-1)) && (yPosVector == 0)
            direction = 1;
            yPosVector = 500;
            pointReached = true;
            pause(0.5);
...
% [minSmoothPointSize, maxSmoothPointSize, minAliasedPointSize, maxAliasedPointSize] = Screen('DrawDots', window, [xPosVector; yPosVector], dotSizes, dotColorsRed, dotCenter, 2);
% % Flip to the screen
% vbl  = Screen('Flip', window, vbl + (waitframes - 0.5) * ifi);
% pause(1);
% 
...
%         % Draw all of our dots to the screen in a single line of code adding
%         % the sine oscilation to the X coordinates of the dots
%         [minSmoothPointSize, maxSmoothPointSize, minAliasedPointSize, maxAliasedPointSize] = Screen('DrawDots', window, [xPosVector + gridXPos; yPosVector + gridYPos], dotSizes, dotColorsRed, dotCenter, 2);
% 
%         % Change direction if maxlimit reached
%         if (direction == 1) && (xPosVector >= maxLimit) && (yPosVector == (maxLimit*-1))
%             direction = 2;
